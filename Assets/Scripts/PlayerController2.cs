...
public class PlayerController2 : MonoBehaviour
{
	...
	private void CheckArm()
	{
		...
		//      Limits: max 90 --> 121
		// Hand: Limit Max: 90 --> 0
		if (attackState == State.Meleeing || normalState == State.Holding || attackState == State.Blocking) return;
	...
	private void MeleeAlternateSchemaHelper()
	{
		...
		if (Physics.SphereCast(transform.position, CharacterDataStore.CharacterMeleeDataStore.PunchRadius, transform.forward, out hit, CharacterDataStore.CharacterMeleeDataStore.PunchDistance, layermask))
		{
			...
			hit.transform.GetComponentInParent<PlayerController2>().OnMeleeHit(force, MeleeCharge, gameObject);
			hit.transform.GetComponentInParent<PlayerController2>().Mark(gameObject);
		}
	}
