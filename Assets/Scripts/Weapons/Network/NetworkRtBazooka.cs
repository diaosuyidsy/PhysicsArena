...
public class NetworkRtBazooka : NetworkWeaponBase
{
    [HideInInspector]
    ...
    [SyncVar]
    private float _HLAxis;
    [SyncVar]
    private float _VLAxis;
    ...
    [TargetRpc]
    private void TargetBomb(NetworkConnection connection, GameObject target, Vector3 force, GameObject owner)
    {
        target.GetComponent<IHittableNetwork>().OnImpact(force, ForceMode.Impulse, owner, ImpactType.BazookaGun);
    }
    ...
    [Command]
    private void CmdSetAxis(float HLValue, float VLValue)
    {
        _HLAxis = HLValue;
        _VLAxis = VLValue;
    }

    // Aim is called on both clients and server
    private void _aim()
    {
        if (_ownerIsLocalPlayer)
        {
